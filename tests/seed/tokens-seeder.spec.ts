...
import { tokenMaster, TOKENS_MASTER_DEPLOY_VALUE, TOKENS_MINT_FORWARD, TOKENS_MINT_TOTAL } from '../../scripts/seed/tokensModule';
...
describe('tokens seeder — deploy master + mint to recipient', () => {
    ...
    it('mints exactly 1,000,000 raw units of token A to the recipient', async () => {
        ...
        // Deploy with the seeder's REAL (trimmed) value — proves 0.25 covers deploy + storage.
        const dep = await master.sendDeploy(admin.getSender(), TOKENS_MASTER_DEPLOY_VALUE);
        ...
        await master.sendMint(admin.getSender(), recipient.address, AMOUNT, TOKENS_MINT_FORWARD, TOKENS_MINT_TOTAL);
    ...
    it('re-mint tops up a partial balance to exactly the target amount', async () => {
        ...
        await master.sendDeploy(admin.getSender(), TOKENS_MASTER_DEPLOY_VALUE);

        // first mint a partial amount, then top up the shortfall (what the idempotent seeder does)
        await master.sendMint(admin.getSender(), recipient.address, 400_000n, TOKENS_MINT_FORWARD, TOKENS_MINT_TOTAL);
        const wAddr = await master.getWalletAddress(recipient.address);
        const wallet = blockchain.openContract(JettonWallet.createFromAddress(wAddr));
        const bal = await wallet.getJettonBalance();
        await master.sendMint(admin.getSender(), recipient.address, TARGET - bal, TOKENS_MINT_FORWARD, TOKENS_MINT_TOTAL);
